...
const sprites = [
  ...
  },
  {
    ...
    url: "https://raw.githubusercontent.com/Dixper/dixper-sdk-plugin-samples/xmas-david/src/xmas/assets/spritesheets/2xMesa_de_trabajo_39.json",
  },
  // {
...
const sounds = [
  "https://raw.githubusercontent.com/Dixper/dixper-sdk-plugin-samples/main/src/xmas/assets/sounds/Box_opening.mp3",
...
//  mouse = new dxCursor(DX_PIXI, "crosshair", DX_LAYERS.cursor, {
...
const createCrate = () => {
  ...
  orderPrice.forEach((price, index) => {
    surpriseBox = new DxButton(
      "https://raw.githubusercontent.com/Dixper/dixper-sdk-plugin-samples/xmas-david/src/xmas/assets/spritesheets/GIFT.png",
      "",
      {
        isClickable: true,
        controller: {
    ...
    crates.onClick = (event) => {
      if (!checkReward) {
        ...
        if (typeof crates._options.priceCrate === typeof String()) {
          timer.remove(false);
          dixperPluginSample.addParentSkill("nTIHFh7Jr2DKEob66sWQ");
...
const createChallengeSuccess = (price) => {
  ...
  let tempTimeout = setTimeout(() => removeSuccess(), 7000);
  timeoutArray.push(tempTimeout);
  tempTimeout = setTimeout(() => clearTimeouts(), 7500);
  timeoutArray.push(tempTimeout);
};
